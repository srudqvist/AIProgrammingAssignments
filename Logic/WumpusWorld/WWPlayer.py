...
class JimmyDean(Player):
    ...
    # def shortPath(self, square, pathBack):
    #     betterPath = []
    #     pathBack = pathBack
    #     neighbors = self.kb.getNeighbors(square)
    #     thisNum = square.col + square.row
    #     for i in range(0, len(neighbors)):
    #         neighborNum = neighbors[i].col + neighbors[i].row 
    #         if neighborNum < thisNum and neighbors[i] in pathBack:
    #             betterPath.append(neighbors[i])
    #             print("appended:", neighbors[i].col, neighbors[i].row)
    #             #pathBack.pop(neighbors[i])
    #             break
    #     while neighbors[i].col + neighbors[i].row != 0:
    #         return betterPath.append(self.shortPath(neighbors[i], pathBack))
    ...
    def reason(self,sensorData):
        ...
        if not breeze and not stench:
            for neighbor in neighbors:
                if not neighbor.visited not in self.kb.safeNotVisited:
    ...
    def selectAction(self):
            ...
            if self.col == 0 and self.row == 0:
                ...
                if not self.givingUp:
                    ...
                    for square in self.visitedSquares:
                        ...
                        self.pathBack.append(square)
                    self.pathBack.reverse()
                    [self.res.append(x) for x in self.pathBack if x not in self.res]
                    for i in range(0, len(self.pathBack)):
                        print("PathBack:", self.pathBack[i].col, self.pathBack[i].row)
                    for i in range(0, len(self.res)):
                        print("res:", self.res[i].col, self.res[i].row)
                    #betterPath = self.shortPath(currentKBSquare, self.pathBack)
                ...
                if self.givingUp:
                    if self.col == 0 and self.row == 0:
                        ...
                        #while nextMove == self.lastSquare:
                         #   nextMove = self.pathBack.pop(0)
        ...
        if self.hasGold:
            ...
            if self.col == 0 and self.row == 0:
                ...
                print(currentKBSquare.col, currentKBSquare.row)
                print(self.hasGold)
                print(self.climb)
                return "climb"
            else:
                ...
                #print(self.res[0].col, self.res[0].row)
                #neighbors = self.kb.getNeighbors(currentKBSquare)
                #num = currentKBSquare.col + currentKBSquare.row
                #for neighbor in neighbors:
                 #   if neighbor.safe and neighbor.visited:
                  #      neighborNum = neighbor.col + neighbor.row
                   #     if neighborNum < num:
                    #        nextMove = neighbor
                # for i in range(0, len(self.res)):

                #     if self.res[i] in neighbors:
                #         nextMove = self.res.pop(i)
                #         break
                #     else:
                #         for neighbor in neighbors:
                #             if neighbor.visited and (neighbor.col + neighbor.row < num):
                #                 nextMove = neighbor
                #         if not nextMove:
                #             nextMove = neighbors[0]
                 #north, south, west, east
                # if len(neighbors) == 4:
                #     print("we have 4 neighbors")
                #     if neighbors[2].safe:
                #         nextMove = neighbors[2]
                #     elif neighbors[0].safe:
                #         nextMove = neighbors[0]
                #     elif neighbors[]
                nextMove = self.pathBack.pop(0)
                #while nextMove == self.lastSquare:

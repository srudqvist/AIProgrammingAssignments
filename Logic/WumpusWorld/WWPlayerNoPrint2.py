        ...
        if len(neighbors) == 2:
            if neighbors[0].visited and neighbors[1].visited:
                if neighbors[0].breeze and neighbors[1].breeze:
                    self.climb = True
                    return "climb"
        # If we miss the wampus and there is something blocking us from exploring
            elif self.wumpusLocated and self.kb.wumpusInfo[0] == neighbors[0]:
                if currentKBSquare.col == 0 and currentKBSquare.row == 0:
                    if neighbors[0].visited:
                        self.climb = True
                        return "climb"
            elif self.wumpusLocated and self.kb.wumpusInfo[0] == neighbors[1]:
                if currentKBSquare.col == 0 and currentKBSquare.row == 0:
                    if neighbors[1].visited:
                        self.climb = True
                        return "climb"
